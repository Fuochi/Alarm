...
static void unicast_recv(struct unicast_conn *c, const linkaddr_t *from) {
	...
	if (MOTE_ID != *((uint8_t*) packetbuf_dataptr() + DEST_ID_INDEX)) {
		...
		case UNI_CONFIG:
			...
			if ((*((uint8_t*) packetbuf_dataptr() + SOURCE_ID_INDEX)))
			DistanceSensor_init(NO_SENSOR);
			else
...
PROCESS_THREAD(example_broadcast_process, ev, data) {
	...
	while (1) {
		...
		// check if we have data in input from the usb
		usb_read_packet (1, &val, 0x1);
		if(!val)
		{
			if (val>100)
			{
				package.source_id = OFF;
				package.dest_id = val-100;
			}
			else
			{
				package.source_id = ON;
				package.dest_id = val;
			}

			package.type = UNI_CONFIG;
			((uint8_t*) &package)[UNI_HOPCOUNT_INDEX] = routingTable[searchMote_Index][ROUTING_HOPS_INDEX];
			uint8_t i;
			for (i = 0; i < routingTable[searchMote_Index][ROUTING_HOPS_INDEX]; i++) {
				((uint8_t*) &package)[PATH_INDEX + i] = routingTable[searchMote_Index][ROUTING_PATH_INDEX
